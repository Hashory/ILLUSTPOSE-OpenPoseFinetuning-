...
def NMS(
	param, heatmaps, upsampFactor=1.0, bool_refine_center=True, bool_gaussian_filt=False
):
	"""
...
def find_connected_joints(
	param, paf_upsamp, joint_list_per_joint_type, num_intermed_pts=10
):
	"""
...
def plot_pose(
	...
	to_plot = (
		canvas.copy()
		if bool_fast_plot
		else cv2.addWeighted(img_orig, 0.3, canvas, 0.7, 0)
	)
	...
	for limb_type in range(which_limbs_to_plot):
		for person_joint_info in person_to_joint_assoc:
			...
			polygon = cv2.ellipse2Poly(
				coords_center,
				(int(limb_length / 2), limb_thickness),
				int(angle),
				0,
				360,
				1,
...
def decode_pose(img_orig, heatmaps, pafs):
	...
	joint_list_per_joint_type = NMS(
		param, heatmaps, img_orig.shape[0] / float(heatmaps.shape[0])
	)
