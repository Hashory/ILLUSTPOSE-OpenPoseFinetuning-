...
class aug_scale(object):
	...
	def __call__(self, meta_data, img, mask_miss):
		...
		if meta_data["type"] == 1:
			scale_multiplier = (
				self.params_transform["itop_scale_max"]
				- self.params_transform["itop_scale_min"]
			) * dice + self.params_transform["itop_scale_min"]
			scale_abs = self.params_transform["target_dist"]
		else:
			# COCOの場合、ランダムに0.5倍～1.1倍する
			scale_multiplier = (
				self.params_transform["coco_scale_max"]
				- self.params_transform["coco_scale_min"]
			) * dice + self.params_transform["coco_scale_min"]
			scale_abs = (
				self.params_transform["target_dist"] / meta_data["scale_provided"]
			)
		...
		mask_miss = cv2.resize(
			mask_miss, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC
		)
...
class aug_rotate(object):
	...
	def __call__(self, meta_data, img, mask_miss):
		...
		dice = random.random()  # (0,1)
		degree = (
			(dice - 0.5) * 2 * self.params_transform["max_rotate_degree"]
		)  # degree [-40,40]
		...
		# 画像とマスク画像の回転
		img_rot, R = rotate_bound(
			img, np.copy(degree), (128, 128, 128)
		)  # 回転でできた隙間は青色
...
class aug_croppad(object):
	...
	def __call__(self, meta_data, img, mask_miss):
		...
		if meta_data["type"] == 1:
			x_offset = int(
				(dice_x - 0.5) * 2 * self.params_transform["itop_center_perterb_max"]
			)
			y_offset = int(
				(dice_y - 0.5) * 2 * self.params_transform["itop_center_perterb_max"]
			)
		# ランダムにオフセットを用意 -40から40
		else:
			x_offset = int(
				(dice_x - 0.5) * 2 * self.params_transform["coco_center_perterb_max"]
			)
			y_offset = int(
				(dice_y - 0.5) * 2 * self.params_transform["coco_center_perterb_max"]
			)
		...
		if doflip:
			...
			if meta_data["type"] != 1:
				...
				if num_other_people != 0:
					meta_data["objpos_other"][:, 0] = (
						w - 1 - meta_data["objpos_other"][:, 0]
					)
					meta_data["joint_others"][:, :, 0] = (
						w - 1 - meta_data["joint_others"][:, :, 0]
					)
